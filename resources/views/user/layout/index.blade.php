...
<head>
    @include(config('custom.others.user_theme') . '.layout.head')
</head>
...
<body x-data="main" class="antialiased relative font-nunito text-sm font-normal overflow-x-hidden" :class="[$store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ? 'dark' : '', $store.app.menu, $store.app.layout, $store.app
    ...
    <div class="main-container text-black dark:text-white-dark min-h-screen" :class="[$store.app.navbar]">
        ...
        @include(config('custom.others.user_theme') . '.layout.menu-sidebar')

        <div class="main-content flex flex-col min-h-screen">
            @include(config('custom.others.user_theme') . '.layout.header')

            <div class="p-6 animate__animated" :class="[$store.app.animation]">
                @yield('content')
            </div>

            @include(config('custom.others.user_theme') . '.layout.footer')
        </div>
    </div>
    @include(config('custom.others.user_theme') . '.layout.js')
</body>
