    ...
    <div x-data="invoiceAdd">
        <div class="flex xl:flex-row flex-col gap-2.5">
            <div class="panel px-0 flex-1 py-6 ltr:xl:mr-6 rtl:xl:ml-12">
                <form method="POST" action="{{ route('contact.individual.update') }}">
                    @csrf
                    ...
                    <div class="flex justify-between flex-wrap px-6">
                        ...
                        <div class="lg:w-1/2 w-full lg:max-w-fit">
                            @include(config('custom.others.user_theme') . '.contact.individual.edit-actions')
                        </div>
                    </div>

                    <hr class="border-[#e0e6ed] dark:border-[#1b2e4b] my-6">
                    @include(config('custom.others.user_theme') . '.contact.individual.edit-data')

                    <hr class="border-[#e0e6ed] dark:border-[#1b2e4b] my-6">
                    @include(config('custom.others.user_theme') . '.contact.individual.edit-address')
