@extends(config('custom.others.user_theme') . '.layout.auth', ['LoadScripts' => []])
...
@section('content')
    ...
    <div x-data="auth">
        <div class="absolute inset-0">
            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/bg-gradient.png" alt="image" class="h-full w-full object-cover" />
        </div>
        <div class="relative flex min-h-screen items-center justify-center bg-[url(/assets/images/auth/map.png)] bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16">
            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/coming-soon-object1.png" alt="image" class="absolute left-0 top-1/2 h-full max-h-[893px] -translate-y-1/2" />
            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/coming-soon-object2.png" alt="image" class="absolute left-24 top-0 h-40 md:left-[30%]" />
            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/coming-soon-object3.png" alt="image" class="absolute right-0 top-0 h-[300px]" />
            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/polygon-object.svg" alt="image" class="absolute bottom-0 end-[28%]" />
                ...
                <div class="relative hidden w-full items-center justify-center bg-[linear-gradient(225deg,rgba(239,18,98,1)_0%,rgba(67,97,238,1)_100%)] p-5 lg:inline-flex lg:max-w-[835px] xl:-ms-32 ltr:xl:skew-x-[14deg] rtl:xl:skew-x-[-14deg]">
                    ...
                    <div class="ltr:xl:-skew-x-[14deg] rtl:xl:skew-x-[14deg]">
                        <a href="/" class="w-48 block lg:w-72 ms-10">
                            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/logo-white.svg" alt="Logo" class="w-full" />
                        </a>
                        <div class="mt-24 hidden w-full max-w-[430px] lg:block">
                            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/auth/login.svg" alt="Cover Image" class="w-full" />
                        </div>
                    </div>
                </div>
                <div class="relative flex w-full flex-col items-center justify-center gap-6 px-4 pb-16 pt-6 sm:px-6 lg:max-w-[667px]">
                    <div class="flex w-full max-w-[440px] items-center gap-2 lg:absolute lg:end-6 lg:top-6 lg:max-w-full">
                        <a href="/" class="block w-8 lg:hidden">
                            <img src="{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/logo.svg" alt="Logo" class="w-full" />
                        </a>
                        <div class="dropdown ms-auto w-max" x-data="dropdown" @click.outside="open = false">
                            <a href="javascript:;" class="flex items-center gap-2.5 rounded-lg border border-white-dark/30 bg-white px-2 py-1.5 text-white-dark hover:border-primary hover:text-primary dark:bg-black" :class="{ '!border-primary !text-primary': open }" @click="toggle">
                                <div>
                                    <img :src="`{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/flags/${$store.app.locale.toUpperCase()}.svg`" alt="image" class="h-5 w-5 rounded-full object-cover" />
                                </div>
                                ...
                                <template x-for="item in languages">
                                    <li>
                                        <a href="javascript:;" class="hover:text-primary" @click="$store.app.toggleLocale(item.value),toggle()" :class="{ 'bg-primary/10 text-primary': $store.app.locale == item.value }">
                                            <img class="h-5 w-5 rounded-full object-cover" :src="`{{ url('') }}/_{{ config('custom.others.user_theme') }}/assets/images/flags/${item.value.toUpperCase()}.svg`" alt="image" />
                    ...
                    <div class="w-full max-w-[440px] lg:mt-16">
                        ...
                        <form action="{{ Route('login') }}" method="POST" class="space-y-5 dark:text-white">
                            ...
                            <div>
                                ...
                                <div class="relative text-white-dark">
                                    <input id="username" name="username" type="username" placeholder="Digite o usuário" class="form-input ps-10 placeholder:text-white-dark" required />
                                ...
                                <li>
                                    <a href="javascript:" class="inline-flex h-8 w-8 items-center justify-center rounded-full p-0 transition hover:scale-110" style="background: linear-gradient(135deg, rgba(239, 18, 98, 1) 0%, rgba(67, 97, 238, 1) 100%)">
                                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                                            ...
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M3.21435 8.32997C3.07435 7.90997 2.99481 7.46133 2.99481 6.99997C2.99481 6.5386 3.07435 6.08996 3.21435 5.66996V3.85632H0.875712C0.40162 4.80133 0.131165 5.87042 0.131165 6.99997C0.131165 8.12951 0.40162 9.19861 0.875712 10.1436L3.21435 8.32997Z"
                                                fill="white" />
                        ...
                        <div class="text-center dark:text-white">
                            Não tem uma conta?
                            <a href="/auth/cover-register" class="uppercase text-primary underline transition hover:text-black dark:hover:text-white">INSCREVA-SE</a>
                        </div>
                    </div>
                    <p class="absolute bottom-6 w-full text-center dark:text-white">
                        © <span id="footer-year">2022</span>. Todos os direitos reservados a <b>{{ config('app.name') }}</b>.
                    </p>
                </div>
            </div>
        </div>
